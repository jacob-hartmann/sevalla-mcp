 ...
 * - No OAuth (API key auth is between server and Sevalla, not client and MCP server)
 */
...
export async function startHttpServer(
  ...
  const sessions = new LRUCache<SessionInfo>({
    ...
    onEvict: (sessionId, session) => {
      console.error(
        `[sevalla-mcp] Evicting session ${sessionId.slice(0, SESSION_ID_DISPLAY_LENGTH)} (max sessions reached)`
      );
      try {
        session.transport.close().catch((err: unknown) => {
          console.error(
            `[sevalla-mcp] Error closing evicted session:`,
  ...
  const cleanupSession = (sessionId: string): void => {
    ...
    try {
      session.transport.close().catch((err: unknown) => {
        console.error(
          `[sevalla-mcp] Error closing session ${sessionId.slice(0, SESSION_ID_DISPLAY_LENGTH)}:`,
          /* v8 ignore next */
          err instanceof Error ? err.message : err
        );
      });
  ...
  const mcpPostHandler: express.RequestHandler = async (req, res) => {
    ...
    try {
      ...
      console.error("[sevalla-mcp] Error handling MCP request:", error);
      if (!res.headersSent) {
        res.status(500).json({
          jsonrpc: "2.0",
          error: {
  ...
  const mcpDeleteHandler: express.RequestHandler = async (req, res) => {
    ...
    try {
      ...
      console.error("[sevalla-mcp] Error handling session termination:", error);
      if (!res.headersSent) {
        res.status(500).json({
          jsonrpc: "2.0",
          error: {
  ...
  app.use(
    (
      ...
      console.error("[sevalla-mcp] Unhandled error:", err);
  ...
  const cleanupInterval = setInterval(() => {
    ...
    for (const [sessionId, session] of sessions.entries()) {
      if (now - session.lastActivity > SESSION_IDLE_TIMEOUT_MS) {
        console.error(
          `[sevalla-mcp] Closing idle session ${sessionId.slice(0, SESSION_ID_DISPLAY_LENGTH)}...`
        );
  ...
  await new Promise<void>((resolve, reject) => {
    const server = app.listen(port, host, () => {
      console.error(`[sevalla-mcp] HTTP server listening on ${host}:${port}`);
      console.error(`[sevalla-mcp] MCP endpoint: http://${host}:${port}/mcp`);
      resolve();
    });
    ...
    const shutdown = (signal: string): void => {
      console.error(
        `[sevalla-mcp] Received ${signal}, shutting down gracefully...`
      );

      server.close(() => {
        console.error("[sevalla-mcp] HTTP server closed");
      });

      clearInterval(cleanupInterval);

      console.error(
        `[sevalla-mcp] Closing ${sessions.size} active session(s)...`
      );
      for (const [sessionId, session] of sessions.entries()) {
        try {
          /* v8 ignore start */
      ...
      setTimeout(() => {
        console.error("[sevalla-mcp] Shutdown complete");
        process.exit(0);
      }, 5000);
    };
